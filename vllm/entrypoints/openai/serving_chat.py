...
class OpenAIServingChat(OpenAIServing):
        ...
        try:
            for i, engine_prompt in enumerate(engine_prompts):
                ...
                if isinstance(sampling_params, BeamSearchParams):
                    ...
                    sharing_cache_salt = (
                        request.cache_salt
                        if getattr(request, "cache_sharing", False)
                        and hasattr(request, "cache_salt")
                        else None
                    )
    ...
    async def release_kv_cache(
        ...
        if len(before_engine_prompts) != 1 or (
            ...
            error_msg = (
                f"engine prompts should be 1, while older is "
                f"{len(before_engine_prompts)} "
        ...
        request_params: list[tuple[_Sequence[bytestr], int]] = []
        try:
            elapsed_tokens = 0
            for i, engine_prompt in enumerate(before_engine_prompts):
                if (
                    not isinstance(engine_prompt, dict)
                    or prompt_token_ids_key not in engine_prompt
                ):
                    continue
                if (
                ...
                sharing_cache_salt = (
                    request.cache_salt
                    if hasattr(request, "cache_sharing")
                    and request.cache_sharing
                    else None
                )
                if sharing_cache_salt is not None:
                    sub_request_id = pack_request_sharing_cache_salt(
